...
//=============================================================================
/*:
* @plugindesc 1.6.1自定义鼠标光标，在悬停中突出菜单项，自定义事件鼠标交互等等！查看帮助。
*
...
* @desc 是否希望使用自定义鼠标光标图像。
* true => ON       false => OFF
* @default false
*
* @param Custom Cursor Image
* @desc 自定义游标的文件名。存放在下面设定的文件夹中
* @default default.png
*
* @param Custom Cursors Folder
* @desc 存储自定义游标的文件夹。必须以前斜线结束，默认为img/cursors/
* @default img/cursors/
*
...
* @desc 当鼠标停留在显示有文本命令的事件上时，显示该指标
*
* @param Transfer Cursor
* @desc 当鼠标停留在载具上时，显示该指标
*
* @param Change Gold Cursor
* @desc 当鼠标停留在改变金钱的事件上时，显示该指标
*
* @param Change Items Cursor
* @desc 当鼠标停留在改变物品的事件上时，显示该指标
*
* @param Change Weapons Cursor
* @desc 当鼠标停留在改变武器的事件上时，显示该指标
*
* @param Change Armors Cursor
* @desc 当鼠标停留在改变盔甲的事件上时，显示该指标
*
...
* @desc 当鼠标停留在显示有文本命令的事件上时，显示该图标
*
* @param Transfer Icon
* @desc 当鼠标停留在载具上时，显示该图标
*
* @param Change Gold Icon
* @desc 当鼠标停留在改变金钱的事件上时，显示该图标
*
* @param Change Items Icon
* @desc 当鼠标停留在改变物品的事件上时，显示该图标
*
* @param Change Weapons Icon
* @desc 当鼠标停留在改变武器的事件上时，显示该图标
*
* @param Change Armors Icon
* @desc 当鼠标停留在改变盔甲的事件上时，显示该图标
*
* @param ---Hover Select---
* @desc 我是分割线
* @default
...
* @desc 当上面的数据为true时，移动选择框的se播放的冷却时间，默认为4
* @default 4
*
...
* @desc 用于禁用此事件或事件页上的自动游标切换的注释【使上面那几行cursor无效】
* @default no_auto_cursor!
*
* @param No Auto Icon Notetag
* @desc 用于禁用此事件或事件页上的自动图标切换的注释【使上面那几行icon无效】
...
*
* @param Icon Tag 1
* @desc 设置一个图标标签快捷方式，搭配上面的Change xxx Icon使用。详见网站
* @default quest: 191
*
* @param Icon Tag 2
* @desc 设置一个图标标签快捷方式，搭配上面的Change xxx Icon使用。详见网站
...
* 插件信息
* =~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~
* TDDP鼠标系统是在你的游戏修改鼠标的交互方法的集合。您可以设置自定义鼠标光标，
* 在鼠标悬停事件时显示图标，用鼠标激活事件，等等。
* 有关更新和使用说明的文档，请访问插件的网站:
* http://mvplugins.tordamian.com/?p=26
* 在那里你还可以下载一个PDF文档供离线使用，并且在一个整洁(？)的地方来存放文
* 档，来确保它是最新版本
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
* 服务条款
* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
* 本插件是否商用都免费，从下面的网址查看全部使用条款
* http://mvplugins.tordamian.com/terms-of-use 
* 非常感谢degica制作这个插件免费供大家商业使用！
*/
//=============================================================================
        ...
        Game_Event.prototype.setupMouseSystemProperties = function() {
            ...
            if (this.TDDP_MS.allowAutoCursor) {
                for (var i=0, max=TDDP_MouseSystemEx.autoCursorFilters.length; i < max; i++) {
                    ...
                    var entry = TDDP_MouseSystemEx.autoCursorFilters;
                    var cursor = entry[0];
                    var filter = entry[1];
            ...
            if (this.TDDP_MS.allowAutoIcon) {
                for (var i=0, max=TDDP_MouseSystemEx.autoIconFilters.length; i < max; i++) {
                    ...
                    var entry = TDDP_MouseSystemEx.autoIconFilters;
                    var icon = entry[0];
                    var filter = entry[1];
